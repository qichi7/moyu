...
**核心主旨（不可违背）**：**零干预观察者模拟**——玩家无法对世界演化产生任何影响，只能观察：所有建造/开荒/架桥/探索/命名均由小人自主完成，无指令 UI、无存档干预。**开发约束**：未来任何「玩家干预世界」的功能（指令/直接建造/事件触发等）都违背主旨，**不得主动实现，必须交用户拍板**。
...
node dev/test/entry.js   # 入口冒烟：stub DOM 跑构建产物启动链 + demo 契约/打断切换/沙盘绘制扫描 + 更新日志/顶栏结构存在性，24 项断言
node dev/test/voyage.js  # 航海专项：满载返航 / 低补给被困救援闭环 / 船穿桥 / 岛际大桥立项与贯通，25 项断言
node dev/test/creature.js # 动物专项：老死 / 搁浅死亡与救援 / 繁衍 / 渔场 / 渔船 / 挖塘 / 运鱼 / 灌溉 / 岛数随机 / 牧场门与水生圈养排除 / 散养回栏与门通行 / 动物育龄 / deathReason 标注 / 回栏受阻退避 / 马驯化与 riddenBy 冻结 / 动物迁徙/生态均衡与保护线，107 项断言
...
node dev/test/pixel.js   # 渲染专项：软件光栅化跑 drawScene 与全套 sprite 烘焙（含 riderSprite 两帧与天象气候贴花层），79 项断言
...
node dev/test/mood.js    # 心情专项：衰减/乐事画像/邻近快乐/啤酒档差分/抑郁闭环/找乐子/宴席/余韵/迁居/意外死亡三分支/骑乘端到端/流星/气候带/洋流/台风/均衡出生公式，71 项断言（依赖 .tmp_logic.js）
node dev/test/species.js # 物种专项：新物种表/栖息地/上限守卫/驯化泛化/珍稀渔获/月光花光环/海豚追随，26 项断言（依赖 .tmp_logic.js）
node dev/test/park.js    # 娱乐专项：立项门槛/完工 tile/游乐园圈地浮台/设施补建/游玩闭环，30 项断言（依赖 .tmp_logic.js）
...
| config.js | 全部常量：tile 表 `T`+`TILE_META`（24 种，新增水井/酒坊/压榨坊/烘焙坊）、模拟参数 `SIM`——含**口渴/饮品链**（`THIRST_DECAY`、状态衰减系数 `STATE_HUNGER/ENERGY/THIRST`、`DRINK_RESTORE`、`DRUNK_TIME/DRUNK_SPEED_FACTOR/BEER_HUNGER_FACTOR`、`COFFEE_TIME/COFFEE_ENERGY_FACTOR`、`DEHYDRY_TIME`）与**咖啡田**（`COFFEE_MATURITY/COFFEE_YIELD`）、物种寿命 `SPECIES_AGE`、时代 `ERAS`、航海参数、背包 `PACK_RESTOCK/PACK_LOW/PACK_STACK/PACK_RESTORE/PACK_TOOL_BONUS/PACK_JUICE_ENERGY`、**乐事画像**（`JOY_KEYS/JOY_NAMES/JOY_TIERS/JOY_SALTS/JOY_HOBBY_MIN`）与**骑乘**（`RIDE_SPEED_FACTOR/RIDE_MOUNT_DIST/RIDE_MOUNT_R`）、**天象/迁徙/气候（v0.6.14~16）**（`METEOR_GAP_MIN/MAX 180~360·METEOR_DUR 3·METEOR_MOOD 0.3`、`MIGRATION_PERIOD_MIN/MAX 4×90~6×90=360~540（v0.6.17 修正单位错误）·MIGRATION_DIST_MIN/MAX 30~80`、`CLIMATE{EDGE_INSET 10,SNOW_FARM 0.85,SNOW_BERRY 0.8,DROUGHT_FARM 0.8,DROUGHT_BERRY 0.7,WOBBLE 20,FADE 30,ICE_P 0.55,SNOW_P 0.5,DRY_P 0.5,ICE_SHIP 0.85}`、`CURRENT_SHIP_FAST 1.15/CURRENT_SHIP_SLOW 0.9`、`TYPHOON_R 12/TYPHOON_SHIP 0.6/TYPHOON_MAN 0.85/TYPHOON_CHANCE 1/20000 每秒/TYPHOON_LIFE_MIN/MAX 60~120`）、**生态平衡（v0.6.17）**（`WILD_BREED_CAP 120` 陆生系护栏·`WILD_BREED_SAFETY 1.25` 均衡系数·`SPECIES_CAP` 补 goat 40/deer 20/boar 16/wolf 6·`ECO_FLOOR 4` 狩猎保护线）、**时代节奏（v0.6.18）**（`SETTLEMENT_SCORE [12,36,90]`、`EXPAND_EVERY 30`） | 调数值先看这里 |
...
| world.js | chunk 系统、`generateRegion`（两遍生成：海拔场+悬崖/洞穴，岛公式+发现网格+点亮）、`carveRiver` 河流（逐格登记 `world.rivers` 惰性 Set，河流信息框/渲染/直饮判定用）、`expand` 扩张、tile 读写、资源库存（`ensureStock` **9 字段**：wood/stone/sand/food/water/juice/beer/coffee/beans，缺失字段统一补 0）与联合库存、世界纪事 `logMsg`（每条递增单调计数 `world.logSeq`——logs 截断 200 条后 length 恒定，HUD 判刷新必须用 logSeq）、**气候与洋流（v0.6.15/16）**：`climateAt(x,y)` 按已定居区（world.houses 包围盒）南北缘划带（雪/旱/温带，缓存三失效=chunks 换图/60s 兜底/houses 数变动）——**v0.6.16 边界噪声曲线化**：`snowEdgeY/droughtEdgeY(x) = 基线±EDGE_INSET(10) + _climWobble(x)`（world.js:124-125；`_climWobble=(fbm(x/48+sx,sy,3)−0.5)×2×WOBBLE(20)`，盐值 snow `(137.5,711.3)`/drought `(919.1,313.7)` 两维错相位）、`climateIntensity(x,y)`（world.js:134）连续强度场 0~1（离边界深度，`FADE` 30 格线性爬坡、温带 0，x,y 二元依赖不可缓存）、`currentAt` 16×16 海域块 hash 八向纯函数流场、`shipEnvMul` 洋流顺逆（|dot|>0.25 分档 ×1.15/×0.9）+台风圈内减速 ×0.6+**浮冰海面减速 ×ICE_SHIP 0.85（水面且强度 ≥0.5，world.js:161）**——shipTick **五处乘点**（远航出海/返航/救援 + 渔船出海/归航）共用；`world.meteor/meteorNext/storm` 天象状态字段（genWorld 重置） | `tileAt/setTile/findSpot/ensureStock/revealArea/jointStock/jointConsume/carveRiver/logMsg/climateAt/climateIntensity/currentAt/shipEnvMul` |
...
| creature.js | 十种生物实体（栖息地系统 `habitatOk`：游荡/逃跑疲劳/跟随/驯化/圈养/繁殖/狼捕食；野生繁衍**均衡化（v0.6.17）**：`wildBreedTick`（creature.js:545）每物种期望出生 = 种群×120÷寿命秒×`WILD_BREED_SAFETY 1.25`（:579），floor+概率进位、钳到 `SPECIES_CAP`（turtle/whale 折算专键、无表物种护栏兜底），计数口径非死亡+非圈养（**驯服计入**——驯化不掏空繁衍池）、亲代育龄内野生优先、选址失败少生不硬造；老死；搁浅被困死亡——施工改变地形后等待救援或死亡，`carriedBy` 被人搬运）+ **四方向朝向**（`updateFace` 走统一入口 `faceTurn` 持锁迟滞，按实际位移主轴更新、被挡不动保持；逃跑/游荡/跟随各移动路径实际位移才更新）+ **散养/育龄/死因（v0.6.2）**：`breedAgeOk` 幼年不育/老年停育（无寿命表物种不限，四处亲代池过滤）、updatePasture 散养 roam 11/4 + 离心 >6 确定性回栏 + moveBy 放行 GATE 门格、死亡点标注 `deathReason`；**回栏退避与马（v0.6.8/v0.6.10）**——`pasturePassable` 圈养通行统一判定（栖息地+GATE+栏心，moveBy 放行分支同源收敛）、updatePasture 受阻退避 `retMark/retFail`（连续 2 轮位移 <1 格 → 本轮退随机游走）、马 `tamable:true/tameJoy:20` 走 updateDog 泛化驯化、`riddenBy` 非空冻结自主行为（老死骰不掷）、`isWild` 排除已驯化 tamable；**周期迁徙（v0.6.14）**：`migEligible`（物种表 deer/boar/horse/rabbit+whale+bird 且非圈养非驯化）、`tryMigrate`（migNext 到点掷 50%、40 次尝试找 30~80 格外同栖息地目标）、`updateMigration`（直奔目标、被挡 ±90° 切向找 5~10 格中转、到达恢复并排下周期；接管自主行为但搁浅判定保留在 update 上游） | `creatures` 数组、`populateIslandCreatures`、`habitatOk`、`pasturePassable`、`wildBreedTick`、`breedAgeOk`、`migEligible/tryMigrate/updateMigration` |
| agent.js | 小人：个体属性（探索欲/勤劳）、喜好（explore/homebody/animal/fishing/none）、职业（7 种可转职）、状态机、探索旅程、**航海（startVoyage）**、迁居、登岛命名即定居化、年龄死亡；性别（id hash 确定性）/父姓母姓记录/性别化名字池；**口渴与饮品（thirst/drunkT/coffeeT；`DRINK_ORDER` 城库取用顺序 coffee>juice>beer>water；drink 态原地 2 秒结算；`nearestWaterSpot` 水井/河湖直饮找水，圈扫 40 格结果缓存 2s；thirst<8 持续 40s 脱水病倒 `sickSource="thirst"`）**；**四方向朝向**（统一入口 `faceOf/faceTurn` 在 agent.js 顶层定义、creature.js 同作用域共用——`faceTurn` 带持锁迟滞：反向掉头立即切、90° 变向需 `faceHoldT` 满 0.8s（update 累计封顶 1、切换归 0 重计，undefined 视为 1 首设不受锁），`stepAlong` 按实际位移主轴更新 + `faceTo` 开工面向任务格（显式设向绕锁直设并重置计锁））；**随身槽位背包**（4 助手 `packCount/packAdd/packTake/packFree` + `packHaulCount` 为唯一读写入口，资源/工具/补给全走助手）；**乐事画像与骑乘（v0.6.7/v0.6.10）**——`joyProfile` 七乐事 ×5 档（hash2(id,salt) 确定性、主喜好映射键保底 1.4）、`likedJoyOf` 返回乐事键、`scanFlower/scanMountain` 7×7 圈 1s 节流、`nearestJoySpot/goJoySpot`（24 圈 2s 缓存）、decide 2.9 喝酒/赏花/爬山加权找乐子；`mount/mountCd` 自动骑乘（剩余路径 >8 格且 1.5 格内有驯服马 → 上马，离开 walk 态即下马）；**流星心情与台风减速（v0.6.14/15）**——updateMood 内 `world.meteor && !this.sick` 时心情 +`METEOR_MOOD 0.3`/s（睡觉走 voyage 式早退天然排除）、update 内 1s 节流 `stormNear` 缓存 → stepAlong 移速 ×`TYPHOON_MAN 0.85`（只改速度不改决策） | `Agent.update → decide / die / startVoyage` |
| sim.js | 调度核心：`plannerTick`（10 分支需求聚合 + **饮品链立项**（设施 era+人口≥8 门槛、粮仓周边全域唯一；打水/榨汁/酿酒/烘咖啡库存阈值触发；咖啡田扩种须 5 格内有水）+ **粮食产能口径排除咖啡田**（`foodFarms=!f.crop`，开荒线/出生线/进食点统一过滤，边界含等号语义未动）+ 死任务清理 + 亲子确定（**选亲育龄不分性别统一 20~60**，v0.6.4））、劳动力市场 `jobMarketTick`（builder 需求含 4 新工坊）、聚落演化、时代、出生、宴席、死亡清理、船舶 `shipTick`；**咖啡田独立结算**（farmTick：90s 产 2 豆进城市 beans，不产粮；生长 ×climateAt 气候因子 v0.6.15）；**天象调度 `weatherTick`（v0.6.14/15，simUpdate 开头首个 tick）**：台风逐帧漂移 0.5 格/秒+寿命到点消散、流星过期清空并排 2~4 游戏夜冷却（夜间逐秒 2% 掷骰）、台风每秒 1/20000 掷骰于主仓 40~90 格外深海生成；berryTick 浆果再生按格气候概率折算（每 60s 一轮）；**生态与节奏（v0.6.17/18）**——plannerTick 顶部 `ecoCount/ecoOk`（**ECO_FLOOR=4 狩猎保护线，三处立项过滤：HUNT sim.js:274 / 牧场 CAPTURE :299 / 渔场 CAPTURE :316**，只拦新立项在办照常完成）、疆土扩张阈值 config 化 `pop ≥ (expansions+1)×EXPAND_EVERY(30)`（sim.js:778，旧硬编码 20）、聚落演化消费 `SETTLEMENT_SCORE [12,36,90]`（sim.js:858-860） | `simInit / simUpdate / weatherTick / plannerTick / shipTick` |
| render.js | 场景编排：drawScene 贴 sprite 图集 + 动态叠加层（工程进度/昼夜/分区底纹/地区名标注/选中环/瀑布）、**face→view 朝向映射与 left 翻转**（putLayer 包裹分层，影子/光环/状态粒子在翻转外）、**河流（水格三层判定：池塘→河流→海）/咖啡田（farmInfo 缓存 crop 变体）/四工坊（16×24 底部锚定）渲染，全部带契约未落地回退路径**、**醉酒 wobble**（sin 摆动+偶发 stumble，影子不随动）、**小人分层绘制与姿态分发 `agentPoseOf`（走/跑/吃/工具两帧/钓竿浮标/船内划手双桨固定 side/喝水 drink 两帧/酿造 brew 两帧/醉酒 stumble）**、**骑乘渲染分发（v0.6.10：a.mount 且 riderSprite 可用 → 两帧骑手侧视替代小人分层，falsy/缺函数回退原分层路径）**、**建筑分支草底打底（v0.6.6：buildingSprite 前先贴 tileSprite 地面，根治清屏色透黑）与底图地形感知（v0.6.11：parks.water 水上园区设施换 waterSprite 水色底、波纹帧与水格同口径，否则维持草底）**、动物朝向接线（drawC 翻转包裹/鱼群切线朝向）、chunk 缩略图（zoom<0.4）、缩放平滑语义（≥1 关/0.4~1 开）、**天象与气候视觉层（v0.6.14~16）**：`drawSkyEvents` 闭包（台风径向暗圈+3 层旋转弧扇+26 条 hash 雨丝、流星 4 段拖尾+暖晕 night>0.15 才画；远景 zoom<0.4 与主路径昼夜暗化后两处尾声调用）+ **气候要素贴花层（v0.6.16，render.js:396-417：四类 sticker——浮冰/积雪斑/雪帽/枯草斑，概率=强度×基准 P `ICE_P/SNOW_P/DRY_P`，`hash2(x×73+41,y×151)` 确定性掷骰，tile 后建筑前、s≥6 才撒；**整格 tint 已移除**，经 `climateItOf` 退化链 climateIntensity→climateAt 满强度→0）** + 带内气候粒子（雪 30 点/浮尘 15 点，带范围逐 4 行采样 `climateIntensity(x0,yy)>0` 贴住曲线边界）+ 洋流流纹箭羽（zoom≥0.8、6×6 海域采样）——全 hash/waveT 确定性、暂停冻结、falsy 容错退化不画 | `drawScene / agentPoseOf` |
| audio.js | 程序化音效（振荡器合成，无音频文件） | `sfx.play(name)` |
| sprites.js | 像素 Sprite 图集：全部 tile/房屋 24 变体/**小人分层像素系统三视图（`agentLook` 外观随机 + head/body/pants 三层 25 姿态 × 2 性别；body/pants front|back 归一 "vert"（中缝细节）、head front/side/back（双眼/侧脸/后脑）；新增 drink/brew/stumble 姿态）+ 背包 9 种（新增 water/juice/beer/coffee/beans）**/十种动物三视图（`QUAD_VIEW` 四足兽正/背视点阵 + turtle/whale/fish/bird 特例）/4 种船/**`riverSprite`（4 帧偏青绿）/`buildingSprite`（16×24 向上探出）/`coffeeFarmSprite`/`riderSprite`（骑手两帧 gallop，v0.6.10）/气候要素四 sticker（v0.6.16：`iceSprite` :1697 /`snowPatchSprite` :1706 /`dryPatchSprite` :1724 /`snowCapSprite` :1753——每类 3 个 hash 变体透明底惰性烘焙，点阵定死调色板）**/浪花抖动叠加层的 16×16 点阵惰性烘焙；色彩基建（shade/shadeHex/ELEV_TIERS/TILE_ELEV_VARIANTS/POND_VARIANTS/RIVER_VARIANTS）在此 | `tileSprite/waterSprite/pondSprite/riverSprite/houseSprite/buildingSprite/coffeeFarmSprite/propSprite/agentLook/agentBodySprite/agentPantsSprite/agentHeadSprite/agentHeadLieSprite/agentPackSprite/creatureSprite/shipSprite/foamSprite/ditherSprite` |
| demo.js | 机制演示引擎（v0.6.1 迷你沙盘 / v0.6.13 扩容 **26×13 大沙盘**）：`DEMO_TOPICS` **15 主题** = 字符画布 `stage.rows`（STAGE_TILES 31 字符映射，v0.6.13 补 m/f/n=山/果树/月光花；26 列×13 行、template #demo-cv 600×400、.demo-box 940px）+ `actors` 出场角色（man=agentLook 三层小人（`a.scale` 新生儿缩放、`a.mount` riderSprite 骑乘合绘）/ animal=creatureSprite / ship，moves 关键帧移动与朝向镜像）+ `fx`（label/ring 驯化环/heart/cloud/reveal/bridge 链生长/building 落成（HOUSE 特判走 houseSprite）/cycle 轮播高亮/`bar` 状态条 v0.6.13 落地绘制）+ `hud` 覆盖条；加载期自检断言（逐主题 13 行×等宽、actors/fx 坐标 ∈[0,25]×[0,12]，越界 console.error）；播放器 `demoStart`（点击即打断切换）/`demoTick`（倍速 0.5~4×、完成自动打勾）/`demoRefreshMenu`/`demoWelcome`；绘制复用全局 sprite 图集；拼接在 render 之后 main 之前，**不进 .tmp_logic.js**（UI 层，entry.js 有契约+打断+绘制扫描断言） | `DEMO_TOPICS / demoStart / demoTick / demoRefreshMenu / demoWelcome` |
| main.js | 主循环（固定步长，速度 0/1/2/10/100/1000 档）、Pointer Events 相机（拖拽/捏合/点按）、视觉昼夜 `visualTod`、波光时钟 `waveT`、视角跟随、点击拾取、信息面板（**渴值条五档青蓝梯度（thirst 缺失整行隐藏）、状态行醉/咖啡标注、地块「河流 · 水源」判定（池塘优先）、meta 缺失兜底「建筑」**）、居民名册（**库存行含水/饮/酒/咖**）、**世界动态面板**（`collectActivities` 纯派生只读逻辑层 + `updateActivity` 0.6s 节流；九类活动目录，点击行随机 `focusAgent` 定位跟随）、**三面板折叠**（名册/纪事/动态标题 ▾/▸，内存态）、**纪事 HUD 判刷新用 `world.logSeq`（`\|\| logs.length` 容错回退）**、**死亡广播泛化（v0.6.3：信息框打开即广播、收 agent|creature，动物名兜底「一只」+物种名）**、**更新日志弹窗接线（v0.6.5：#changelog-btn/#changelog-modal，复用 help-btn 模式）**、**信息框「最爱：XX」乐事与「骑乘中」标签（v0.6.7/v0.6.10）**、**面板偏移对齐双行顶栏（v0.6.9：信息框最小 top 48→70）** | `focusAgent / handlePick / updateRoster / updateActivity / collectActivities / packGridHtml / followMode / showDeathBroadcast` |
...
- **T2 文档集中更新**：对照 `git diff` 核实代码实况（**不得轻信第一波回报**）后统一改写 README/HANDOFF；禁碰版本历史与「当前版本」行（归主 agent；任务书明确委托时除外，如 v0.6.6~v0.6.10 波次）；禁碰代码；**每次版本发布还须核对游戏内「更新日志」（template.html #changelog-modal）与「玩法教程」（#help-modal）已随版本同步（见 §6.1 强制约定），缺了打回代码侧补**
...
→ node dev/test/entry.js   （入口回归，24 项：启动链 + demo 契约/绘制扫描 + changelog/顶栏结构）
→ node dev/test/voyage.js  （航海回归，25 项）
→ node dev/test/creature.js（动物回归，107 项）
→ node dev/test/explorer.js（探索者回归，6 项）
→ node dev/test/pixel.js   （渲染回归，79 项）
→ node dev/test/family.js  （亲子回归，40 项）
→ node dev/test/pack.js    （背包回归，41 项）
→ node dev/test/drink.js   （饮品回归，28 项）
→ node dev/test/mood.js    （心情回归，71 项）
→ node dev/test/species.js （物种回归，26 项）
→ node dev/test/park.js    （娱乐回归，30 项）
→ open index.html          （运行游戏，只开一次）
```

> `dev/test/milestones.js`：**大事件时间节点探针工具**（默认 seed 42/104729/99991 × 12000 sim 秒，`--end=N` 可拉长窗口；任务完工结算/状态标志置位/纪事文案首现三类口径 + 村庄/城镇/城市升级事件）——实测估算用途，**不进上述回归必跑清单**；跑数与数据收口归主 agent，结果落 README「大事件时间节点（实测估算）」章节。已知结论：era2（城镇线 60 分）在 36000s/33 游戏年内三 seed 均未自然达成——聚落分散是岛际大桥/酒坊/游乐园节点的支配性瓶颈。

**看护矩阵（功能 → 套件，v0.6.0 审计后）**：生存需求=smoke+mood / 心情抑郁=mood / 饮品=drink+mood / 背包=pack / 任务与规划=smoke+park / 航海与船桥=voyage / 岛际大桥贯通=voyage 场景D / 动物生态=creature / 物种扩充=species / 娱乐链=park / 迁居与意外死亡=mood / 亲子=family / 探索者=explorer / 渲染=sprite=pixel / 启动链与演示=entry / 教程与弹窗=entry（stub 级）/ **圈养散养与动物育龄=creature（v0.6.2）** / **人类育龄=family（v0.6.4）** / **更新日志=entry（stub 级，v0.6.5）** / **死亡广播=人工验收（stub 级，v0.6.3）** / **心情乐事与骑乘=mood（v0.6.7/v0.6.10）** / **回栏退避与马驯化=creature（v0.6.8/v0.6.10）** / **骑手 sprite=pixel（v0.6.10）** / **顶栏双行=entry（stub 级）+人工（v0.6.9）** / **水上设施水色底=人工验收（v0.6.11）** / **顶栏美化重排=entry（stub 级）+人工（v0.6.12）** / **迁徙与流星=mood+creature（v0.6.14）** / **气候带与洋流=mood 单元（v0.6.15）** / **台风=人工注入验收（v0.6.15）** / **机制演示 15 主题大沙盘=entry（stub 级）+人工（v0.6.13）** / **气候要素贴花=pixel（渲染冒烟）+人工（v0.6.16）** / **生态均衡=creature+人工探针（v0.6.17）** / **时代节奏=人工探针 milestones（v0.6.18）**。DOM 交互细节（轮播/信息栏布局/讣告展示）为 stub 级覆盖，需人工验收。
...
**强制约定（用户明确要求，v0.6.6~v0.6.10 波次起固化）：每次版本发布，除 README/HANDOFF 外，必须同步更新游戏内「更新日志」弹窗（template.html `#changelog-modal`，纯修复版本按惯例不收录）与「玩法教程」（`#help-modal`）**——归当波代码 agent 落地、T2 收口核对齐全；漏更等同文档未收口。

测试失败的处理顺序：先看是不是**新代码**破坏了既有语义（对照第 4 节死锁表），再考虑断言本身是否需要随设计更新（改断言要说明理由，禁止静默放松）。

## 7. 当前状态快照（2026-09-23 交接 · v0.6.13~v0.6.18 增量）

- **v0.6.18 时代节奏（A+B）**（`SETTLEMENT_SCORE/EXPAND_EVERY`，config.js:74-75）：**聚落升级线下调** [12,60,140]→**[12,36,90]**——繁荣度=辖区(R14) 房×3+田×2（sim.js:851-860 计算与消费不变）：村庄 12≈4 房、**城镇 36≈8 房+6 田**（config 注：人口 35~45 时主聚落自然达成）、**城市 90≈20 房+15 田**（80~100 人可达）；**疆土扩张放缓**：`pop ≥ (world.expansions+1)×EXPAND_EVERY(30)`（sim.js:778，旧硬编码 20）——扩张摊薄放缓，房屋得以在少数聚落攒分。**背景**：旧线下 67 游戏年 era2 不可达（人口 ~55 平台化、4~7 聚落无一攒到 60 分，岛际大桥/酒坊/游乐园全锁死）；下调后预计 **era2 第 5~8 游戏年、era3 第 20~35 年**——待 `milestones.js` 探针复跑收口，README「大事件时间节点」表数据由主 agent 统一刷新（T2 未动表行）
- **v0.6.17 生态平衡**：**wildBreedTick 均衡化**（creature.js:545 重构）——旧版「全局池随机选亲、每 tick 陆生至多 1 只」结构性追不上死亡率（探针实测 33 游戏年 goat/deer/rabbit/boar/fish/shark/bear/crab 灭绝、fox 独占 60）；现**统一可繁衍物种表**（hunt 系+狼+turtle+whale+SPECIES_CAP 全键，flier 不参与）逐物种出生：期望 = `种群×120s÷寿命秒×WILD_BREED_SAFETY(1.25)`（creature.js:579；寿命秒=寿命年×DAY_LEN×YEAR_DAYS，无表物种兜底 12 年），floor+概率进位、`n=min(n, cap−种群)`，选址失败 break 少生不硬造（半径口径沿用：陆 0~3/狼林 0~5 兜底/龟水 0~5/鲸深 0~8/SPECIES_CAP 系 0~6），亲代 `breedAgeOk` **野生优先、无野生取驯服**。**计数口径 `!dead && !pasture`（驯服计入）**——驯化马/独角兽不再掏空野生繁衍池，出生仍落野生。**上限表**：`SPECIES_CAP` 补 **goat 40/deer 20/boar 16/wolf 6**（config.js:96-98），turtle→TURTLE_CAP 12、whale→WHALE_CAP 5 折算统一口径；`WILD_BREED_CAP 60→120`（config.js:90）**降级为陆生系全局性能护栏**（landPop+landBorn≥120 当 tick 跳过剩余陆生繁衍，非生态上限）；**圈养繁殖同受全局 SPECIES_CAP**（creature.js:402，多栏围捕曾顶破物种上限实测 21/20）。**狩猎保护线 `ECO_FLOOR=4`**（config.js:99）：plannerTick 顶部 `ecoCount`（非死亡非圈养、驯服计入）+`ecoOk`（sim.js:266-270），**三处立项过滤**——HUNT（sim.js:274）/牧场 CAPTURE（:299）/渔场 CAPTURE（:316）；在办任务照常完成（只拦新立项）。**迁徙周期单位修正**：`MIGRATION_PERIOD_MIN/MAX = 4×90~6×90 = **360~540 sim 秒**`（config.js:183-184；4~6 游戏月口径不变）
- **v0.6.16 气候视觉重做**（整格 tint/直线分界全移除，`SIM.CLIMATE` 扩 `WOBBLE 20/FADE 30/ICE_P 0.55/SNOW_P 0.5/DRY_P 0.5/ICE_SHIP 0.85`，config.js:193-196）：**噪声曲线边界**——world.js `snowEdgeY/droughtEdgeY(x) = 房屋包围盒基线±EDGE_INSET(10) + _climWobble(x)`（world.js:124-125），`_climWobble=(noise.fbm(x/48+sx, sy, 3)−0.5)×2×WOBBLE`，盐值 **snow `(137.5, 711.3)` / drought `(919.1, 313.7)`**（两维各加常量错相位，参照 currentAt 位移法）；基线缓存三失效不变（chunks 换图/60s/houses 数）。**climateIntensity(x,y)**（world.js:134-140）连续强度场：`y<se → clamp((se−y)/FADE, 0, 1)`、`y>de → clamp((y−de)/FADE)`、温带 0——纯函数 x,y 二元依赖**不可缓存**（雪线随 x 摆动）。**shipEnvMul 浮冰项**（world.js:161）：水面/深海且 `climateIntensity≥0.5` → ×`ICE_SHIP 0.85`（渔船/远航船共用，与洋流/台风乘点叠加）。**sprites 四 sticker**（每类 3 个 hash 变体、透明底惰性烘焙、点阵定死调色板）：`iceSprite`（sprites.js:1697，2 点阵+镜像 v2）/`snowPatchSprite`（:1706）/`dryPatchSprite`（:1724）/`snowCapSprite`（:1753）。**render 贴花层**（render.js:396-417，tile 后建筑前）：s≥6 才撒；概率 = `强度×基准 P`（`ICE_P 0.55` 浮冰/`SNOW_P 0.5` 积雪斑与雪帽/`DRY_P 0.5` 枯草斑、沙 ×0.5），掷骰 `hash2(x×73+41, y×151)`、变体 `hash2(x×73+47, y×151)`——**hash 确定性无每帧随机**（深处浓密、交界犬牙稀疏）；`climateItOf` 退化链：climateIntensity → climateAt（带内满强度）→ 0 不画。**气候粒子带**（render.js:746+）改逐 4 行采样 `climateIntensity(x0,yy)>0`——飘雪/浮尘视觉贴住曲线边界（旧直线行扫描为退化路径）
- **v0.6.15 气候系统**（影响全部压低、阈值 config 化 `SIM.CLIMATE/CURRENT_*/TYPHOON_*`）：**气候带** world.js `climateAt(x,y)`——按**已定居区**（world.houses 包围盒）南北缘划带：`y < minY+EDGE_INSET(10)` → 雪带、`y > maxY−10` → 旱带、其余温带（**口径注意：带含缘内向内 10 格**，不是「缘外 10 格起」）；farmTick 生长 ×`SNOW_FARM 0.85`（雪）/×`DROUGHT_FARM 0.8`（旱，coffee 田同乘）；berryTick 浆果再生**概率折算**（每 60s 一轮、逐格 `rand()<因子`——再生是整份 +1 跳变，期望速率 ×`SNOW_BERRY 0.8`/×`DROUGHT_BERRY 0.7`）；缓存三失效（world.chunks 换图/60s 兜底/houses 数变动）→ **带随疆土漂移**。**洋流** `currentAt` 纯函数（16×16 海域块 `hash2(x/16+511, y/16+977)` 八向单位向量、非水面零向量）→ `shipEnvMul` 航向·流向点积 |dot|>0.25 分档 ×`CURRENT_SHIP_FAST 1.15`/×`CURRENT_SHIP_SLOW 0.9`——**shipTick 五处乘点**（world.js 行 356 远航出海/399 返航/438 救援 + 490 渔船出海/508 渔船归航）+ 渔船台风圈内**歇网**（到点不起网分支）。**台风** sim.js `weatherTick` 内 `world.storm={x,y,dx,dy,born,life,r}`——每秒 `TYPHOON_CHANCE 1/20000` 掷骰、主仓 40~90 格外 DEEP 生成（30 次尝试）、0.5 格/秒漂移、`TYPHOON_LIFE 60~120s` 消散（logMsg「台风在海面上消散了」）；圈内 R12（`TYPHOON_R`）船速 ×`TYPHOON_SHIP 0.6`（shipEnvMul）、行人移速 ×`TYPHOON_MAN 0.85`（agent 1s 节流 `stormNear`）——**只改速度不改决策**。**表现层 render.js**：`drawSkyEvents`（台风径向暗圈+3 层旋转弧扇+26 条 hash 雨丝；远景/主路径两处尾声调用）+ 逐行 climateAt tint（雪 rgba(235,242,250,.22)/旱 rgba(216,190,120,.16)，含建筑格）+ 带内粒子（雪 30/尘 15）+ 洋流流纹（zoom≥0.8、6×6 采样箭羽）——全 hash/waveT 确定性、暂停冻结、falsy 容错
- **v0.6.14 天象与迁徙**（`SIM.METEOR_*/MIGRATION_*`）：**流星**——sim.js `weatherTick`（**simUpdate 开头首个 tick，每 sim 秒一掷骰窗口**）：过期清 `world.meteor={t0,dur}` 并排 `METEOR_GAP 90×2~90×4`（2~4 游戏夜）冷却 → 夜间（!isDaytime）到点后逐秒 2% 概率现身、划空 `METEOR_DUR 3s`；心情：agent.js updateMood `world.meteor && !this.sick` → +`METEOR_MOOD 0.3`/s（睡觉走早退天然排除、病中无暇仰望）。**迁徙** creature.js——`migEligible()` 口径：**物种表（deer/boar/horse/rabbit 陆生兽 + whale + bird）且非圈养且未驯化**（tamable∧tamed 除外；鱼/龟/海豚/狼不动；bird 天然被 isWild 排除但资格按物种表独立判）；`tryMigrate`：`MIGRATION_PERIOD_MIN/MAX 4×90×30 ~ 6×90×30`（**4~6 游戏月**）到点掷 50%、40 次尝试找 `MIGRATION_DIST 30~80 格`外 habitatOk 目标（找不到本轮放弃）；`updateMigration` 接管自主行为（搁浅判定保留在 update 上游）：stepToward 直奔、被挡（moveBy 置空 target 哨兵）→ **±90° 切向 ±side** 找 5~10 格同栖息地中转（10 次尝试）、到达恢复并排下周期；**迁徙照常可狩猎**（isWild 不变）；render.js 流星拖尾 4 段+暖晕（night>0.15 才画）。**【v0.6.17 更正】本条原记 `MIGRATION_PERIOD_MIN/MAX 4×90×30 ~ 6×90×30` 为单位错误**——把 1 游戏月误按 30 sim 秒折算，实际周期曾是设计意图的 30 倍（兽群几乎不迁）；v0.6.17 修正为 `4×90 ~ 6×90 = 360~540 sim 秒`（4~6 游戏月不变，1 游戏月 = DAY_LEN 90s，见 config.js:183-184）
- **v0.6.13 机制演示扩充**：demo.js 全 **15 主题**重排 **26×13 大沙盘**（template #demo-cv **440×280→600×400**、.demo-box **860→940px**）——新增 pack 背包搬运 / frontier 探索迁居 / pasture 散养育龄 / family 亲子育龄 / horse 驯马骑乘 5 主题；口径更新：mood（「工作磨人 / 七类乐事 / 抑郁与走出」+加权找乐子+「最爱」标签）、fate（「讣告打开即播报、动物离世也上讣告」文案+hud 讣告卡）、park（「海上浮台的水色就是设施的底色」）；**引擎小修**：STAGE_TILES 补 `m`/`f`/`n`（MOUNTAIN/FRUIT/MOONBLOOM）、`bar` 状态条 fx 落地绘制（dBar2，原只有数据从未渲染）、dMan2 骑手分支（o.mount→riderSprite 合绘）、HOUSE 特判 houseSprite（28 高底锚——buildingSprite 无 HOUSE 分支）、浆果/果树走 propSprite 带果档、`a.scale` 新生儿小号缩放；**加载期自检断言**（逐主题 13 行×等宽、actors/fx 坐标 ∈[0,25]×[0,12]，越界 console.error——非测试断言，不进回归计数）
- **v0.6.12 顶栏美化重排**：template.html 顶栏**纯间距分组**——上行 #top-info：品牌组 .grp-brand（标题 + #build-id **版本徽章胶囊**：border #3a4468 / color #8f9ab0 / 圆角 999px，手机端随标题一起隐藏）→〔人口·存粮·房屋〕〔历法·时代〕〔实际速率行末〕；下行 #top-ctl：左侧视角组 .grp-view（回原点 + #zoom-ctl 滑轴，**margin-right:auto**）+ 右侧〔⏸暂停+#speed-ctl 速度档〕〔音效〕〔教程·日志·▶演示〕；间距口径：组间 18px（外层 gap）/ .grp 组内 10px（信息行）· 5px（操作行，视角组 8px）；按钮统一 `#top-ctl button` **高 24px**（圆角 4px/字号 12px，hover/active 沿用 v0.6.9 配色）；**#speed-ctl 容器拆分**——回原点拆出独立视角组、#speed-ctl 仅剩暂停+速度档，**main.js 无 #speed-ctl JS 引用**（grep 核实仅 go-home/build-id/stat-era 等按 id 引用，id 全保留）；.grp/#zoom-ctl/#speed-ctl **div→span** 合法性调整；媒体查询收紧 + flex-wrap；**面板偏移不变**（roster/activity top 78·112、death-broadcast 84、信息框 70——v0.6.9 数值沿用）；help-modal 新增「顶栏操作」段、#changelog-modal 补 v0.6.12 条（v0.6.11 按惯例纯修复不进游戏内 changelog）
- **v0.6.11 水上设施水色底**：render.js 建筑分支（井/酒坊/压榨坊/烘焙坊 + 凉亭/戏台/斗兽场/门楼/摩天轮/旋转木马/过山车）底图**地形感知**——逐个扫 `world.parks` 注册表（`{x,y,x0,y0,x1,y1,water}`，tasks.js 游乐园完工时登记）找 `water:true` 且格在 bounding 内 → 底图换 `waterSprite(tile, eTier, v, ((wFrameBase+h*4)|0)%4)`（**波纹帧与下方水格分支同口径**：wFrameBase/h 同作用域、h=hash2(x,y)；本分支 tile 非 T.DEEP，waterSprite 走浅水烘焙），否则 tileSprite 草底（v0.6.6 兜底）；背景：水上乐园设施完工时 setTile 把浮台 PIER 覆盖为 FERRIS 等 tile，v0.6.6 草底兜底曾令水上设施垫草绿；**口径边界**：parks 注册表支撑水/草两态已覆盖全部实际场景（工坊/凉亭只立陆地），「任意原地形记录」未实现（需 setTile 层记录、改动大不做）；有/无 buildingSprite 两条分支均先铺 groundSpr 再画建筑
- **v0.6.10 马与骑乘**：CREATURE_META.horse 加 `tamable:true / tameJoy:20`（走 updateDog 泛化驯化路径）；agent 侧骑乘三常量 `RIDE_SPEED_FACTOR 1.8 / RIDE_MOUNT_DIST 8 / RIDE_MOUNT_R 1.5`——walk 态剩余路径 >8 格且身旁 1.5 格内有自己驯服的空载马 → 自动上马（1s 节流 mountCd，零 rand 消耗），坐标每帧由骑手同步；**离开 walk 态（到达/改道/开工）即下马**；creature 侧 `riddenBy` 非空 = 自主行为全冻结（游荡/驯化扫描跳过、老死骰不掷——骑乘中不会中途倒下，年龄照常增长）；**isWild 追加排除已驯化 tamable**（驯化独角兽不再进野生繁衍池/狩猎捕获池——既有隐患顺带修复；未驯化的 tamable 仍算野生）；sprites `riderSprite(frame 0/1)` 两帧骑手侧视（left 镜像、up/down 复用 side）+ render 分发（mount falsy 或函数缺失回退原小人分层路径）；main 信息框「骑乘中」标签
- **v0.6.9 顶栏双行**：#ui-top 改 flex-direction column 两行——上行 **#top-info**（标题 + 人口/存粮/房屋/历法/时代/实际速率 + #build-id 版本号）、下行 **#top-ctl**（缩放滑轴 + 全部按钮）；.stat 定宽类 **w-pop 64 / w-food 78 / w-house 64 / w-date 150 / w-era 110 / #rate-box 110** + nowrap + overflow hidden——数字变长不推挤换行；面板偏移随行高调整：roster/activity top 54→**78**、death-broadcast 64→**84**、信息框最小 top 48→**70**（main.js 三处）；手机端两行各自可换行、roster/activity top 92→112
- **v0.6.8 圈养回栏受阻退避**：v0.6.2 确定性回栏被山/树挡住时 moveBy 置空 target → 下轮重选同一目标**无限顶墙**（用户实测卡死）——updatePasture 加 `retMark/retFail`：回栏轮记录起点，下轮位移 <1 格计一次失败，**连败 2 → 本轮退随机游走**（retMark 清空、retFail 归 0，即 2 败 1 随机循环）；目标校验统一 `pasturePassable`（`habitatOk ∥ 圈养∧GATE/PASTURE`），moveBy 圈养放行分支同源收敛，非圈养生物语义逐位不变
- **v0.6.7 心情乐事体系**：`SIM.JOY_KEYS` 七乐事（flower 赏花/drink 喝酒/explore 探险/climb 爬山/fish 垂钓/animal 牧畜/home 恋家）× `JOY_TIERS` [0.4, 0.7, 1.0, 1.4, 1.8]，`JOY_SALTS` 逐键盐值 hash2(id,salt) 确定性分档（**零 rand 流**）；`JOY_HOBBY_MIN 1.4`——主喜好映射键（_HOBBY_JOY：explore→explore / homebody→home / animal→animal / fishing→fish；none 无映射无保底）保底 1.4；`likedJoyOf` 改返回乐事键（updateMood 按 `joyProfile[键]` 倍率回甘）；**出海加成全民放开**（voyage 早退前 `MOOD_LIKED_JOY × joyProfile.explore`；搭桥/远行仍限向往远方喜好）；麦酒回甘 ×joyProfile.drink（背包自用 + 城库饮用两处）；赏花/爬山 = `scanFlower/scanMountain` 7×7 圈 1s 节流（花=T.BERRY/FRUIT/MOONBLOOM、山=T.MOUNTAIN/CLIFF，tile 未接入容错 false）；decide 2.9 找乐子改喝酒/赏花/爬山三候选按档位**加权无放回抽样**（此处允许消耗 rand 流；nearestJoySpot 24 圈 2s 缓存，花丛站目标格/山站邻近可走格），选中项落地失败依次回退、全落空 JOY_CD 冷却；main 信息框性格行「最爱：XX」（最高档键，并列优先主喜好键）
- **v0.6.6 娱乐建筑草底**：render.js 建筑分支（井/酒坊/压榨坊/烘焙坊 + 凉亭/戏台/斗兽场/门楼/游乐园设施）画 buildingSprite 前**先贴 tileSprite 地面打底**——tileSprite 对未覆盖 tile 兜底烘焙草地绿（sprites.js bakeGround `#5e8c4f` 系），根治清屏色 `#05070c` 透过 sprite 透明像素的「凉亭黑底」
...
- 已知边界：各城粮仓无上限（宴席是主要出口）、无存档、小人死亡无墓地实体、野生繁衍已均衡化（v0.6.17：每物种 `SPECIES_CAP` 专键 + 陆生系全局护栏 `WILD_BREED_CAP=120`，旧 60 全局共享口径作废）、1000× 时帧率受模拟步进限制（每帧 1200 步上限）、航海撞岸点若无 walkable 邻居则转向绕行（水手不硬着陆）
