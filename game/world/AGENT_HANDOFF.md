...
- **粮食城市内共享**：`settlement.stock.food`（`ensureStock` 兜底），产出（农田/牧场/渔船/采集）进 `ownerSettle`（无半径最近聚落），进食只吃所在城市库存，出生检查出生城市存粮 ≥20，宴席由存粮最多的城市承担；HUD/闸门/告警用 `totalFood()`（全局视角指标）；world.food 已移除
